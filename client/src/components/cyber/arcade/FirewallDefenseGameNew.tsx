...
const FirewallDefenseGameNew: React.FC<FirewallDefenseGameProps> = ({ difficulty, onGameEnd }) => {
  ...
  const runSimulation = () => {
    // Créer une liste des attaques qui vont se produire
    const simulatedAttacks = attackTypes.filter(attack => 
      // Probabilité de l'attaque en fonction de la difficulté
      Math.random() * 10 < attack.probability
    );
    
    // Pour chaque attaque, déterminer si elle est bloquée
    const attackResults = simulatedAttacks.map(attack => {
      ...
      for (const defenseId of targetZone.defenses) {
        ...
        if (Math.random() * 10 < effectiveness) {
          isBlocked = true;
    ...
    const successfulAttacks = attackResults.filter(ar => !ar.blocked).length;
    ...
    if (successfulAttacks === 0) {
      feedback = "Excellent travail ! Votre stratégie de défense a bloqué toutes les attaques. Votre infrastructure est bien protégée.";
    } else if (successfulAttacks < attackResults.length / 3) {
      feedback = "Bonne défense globale, mais quelques attaques ont réussi à passer. Renforcez les zones vulnérables.";
    } else if (successfulAttacks < attackResults.length / 2) {
      feedback = "Défense moyenne. Un nombre significatif d'attaques ont réussi. Reconsidérez votre stratégie de placement des défenses.";
    } else {
      feedback = "Défense insuffisante. La majorité des attaques ont réussi. Votre infrastructure est gravement compromise.";
    }
    ...
    // Appeler le callback de fin de jeu si fourni
    if (onGameEnd) {
  ...
  const renderSimulationPhase = () => (
    <div className="flex flex-col items-center justify-center min-h-[70vh] p-4">
      ...
      >
        <h2 className="text-2xl font-bold text-white mb-4">Simulation en cours...</h2>
        ...
        <div className="relative w-32 h-32 mx-auto mb-6">
          ...
          >
            <RefreshCw className="w-32 h-32 text-amber-500/20" />
          </motion.div>
          
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-3xl font-bold text-amber-400">{simulationTimeLeft}</span>
          </div>
        </div>
        
        <p className="text-gray-300 mb-4">
          Le système est en train d'évaluer la résistance de votre infrastructure face aux cyberattaques...
        </p>
        
        <Progress value={(10 - simulationTimeLeft) * 10} className="h-2 mb-2" />
        
        <p className="text-sm text-gray-400">
          Les attaquants tentent de compromettre vos systèmes. Vos défenses sont-elles prêtes ?
        </p>
      </motion.div>
    </div>
  );
