...
export default function DataFormationPage() {
  ...
  const runSandboxQuery = async () => {
    ...
    try {
      const res = await apiRequest<SandboxResponse>('/api/formation/sql/sandbox', {
        method: 'POST',
  ...
  const loadNextScenario = async () => {
    ...
    try {
      const res = await apiRequest<{
        ...
        levelLabel: string;
        message?: string;
      }>('/api/formation/sql/next-scenario', {
        method: 'POST',
