...
export default function PythonFormationPage() {
  ...
  const startPythonJourney = async () => {
    ...
    try {
      ...
      }>('/api/client/formation/python/start', { method: 'POST', body: JSON.stringify({ firstName: cleanName }) });
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      }>('/api/client/formation/python/quiz/submit', {
        method: 'POST',
  ...
  const runSandboxCode = async () => {
    ...
    try {
      const res = await apiRequest<SandboxOutput>('/api/client/formation/python/sandbox', {
        method: 'POST',
  ...
  const autoEvaluate = async (code: string, result: SandboxOutput) => {
    ...
    try {
      ...
      const res = await apiRequest<{ success: boolean; assistantMessage: string }>(
        '/api/client/formation/python/assistant',
  ...
  const loadNextScenario = async () => {
    ...
    try {
      ...
      }>('/api/client/formation/python/next-scenario', {
        method: 'POST',
  ...
  const finishSession = async () => {
    ...
    try {
      const res = await apiRequest<{ success: boolean; summary: string; message?: string }>(
        '/api/client/formation/python/summary',
