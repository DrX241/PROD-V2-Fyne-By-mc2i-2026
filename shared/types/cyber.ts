...
export interface CyberDomain {
  ...
  scenarios: string[]; // Références aux IDs des scénarios disponibles dans ce domaine
}
...
export interface CyberScenario {
  ...
  missionTemplate?: Partial<Mission>;
}
...
export interface ChatMessage {
  id: string;
  type: "user" | "bot" | "domain-selection" | "scenario-selection" | "scenario-context" | "email";
...
export interface ScenarioState {
  ...
  contact: ScenarioContact | null;
  scenarioContacts: ScenarioContact[];
  currentMessageIndex: number;
  showingIntro: boolean;
  trustLevel: number; // Niveau de confiance de 0 à 100
  missionProgress: number; // Progression de la mission de 0 à 100
}
