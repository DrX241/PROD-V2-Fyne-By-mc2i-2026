...
Réponds directement sans introduction ni formule de politesse, comme si tu intervenais dans une conversation.`;
  ...
  // API pour les conseils IA dans le jeu Code Breaker
  app.post('/api/cyber/codebreaker/ai-advice', async (req: Request, res: Response) => {
    try {
      const { cipherType, cipherText, userSolution, level = 1 } = req.body;
      
      if (!cipherType || !cipherText) {
        return res.status(400).json({ error: 'Informations de chiffrement manquantes' });
      }
      ...
      const messages: ChatCompletionRequestMessage[] = [
        {
          role: "system",
          content: `Tu es un assistant spécialisé en cryptanalyse qui aide à résoudre des codes chiffrés. 
          Tu dois fournir des indices utiles sans donner directement la solution complète.
          Adapte tes réponses au niveau de difficulté (1-5) et aux progrès de l'utilisateur.
          L'utilisateur soumet un type de chiffrement, un texte chiffré et peut-être une tentative de solution.
          Fournis trois éléments dans ta réponse :
          1. Un indice précis et ciblé pour le chiffrement spécifique
          2. Une brève explication sur le fonctionnement de ce type de chiffrement
          3. Une liste d'étapes concrètes à suivre pour déchiffrer le message`
        },
        {
          role: "user",
          content: `Je travaille sur un chiffrement de type "${cipherType}" de niveau de difficulté ${level}/5.
          Le texte chiffré est : "${cipherText}"
          ${userSolution ? `Ma tentative de solution actuelle est : "${userSolution}"` : "Je n'ai pas encore de solution."}`
        }
      ];
      ...
      // Analyse de la réponse pour extraire les sections
      try {
        // Extraire l'indice, l'explication et les étapes à l'aide d'heuristiques
        const hintMatch = aiResponse.match(/(?:indice|hint).*?[:]\s*(.*?)(?=\n\n|\nexplication|\nfonctionne)/is);
        const explanationMatch = aiResponse.match(/(?:explication|fonctionne).*?[:]\s*(.*?)(?=\n\n|\nétape|\nvoici|\npour|\nprochaine)/is);
        const stepsText = aiResponse.match(/(?:étape|prochaîne|voici.*?étape|pour.*?déchiffrer).*?[:]\s*([\s\S]*?)(?=$)/is);
        
        // Extraction des étapes individuelles
        let steps: string[] = [];
        if (stepsText && stepsText[1]) {
          steps = stepsText[1]
            .split(/(?:\r?\n|\r)/)  // Séparer par lignes
            .filter(line => line.trim().match(/^\d+[\.\)]\s+|^-\s+|^\*\s+|^•\s+/))  // Garder seulement les lignes numérotées ou avec puces
            .map(line => line.replace(/^\d+[\.\)]\s+|^-\s+|^\*\s+|^•\s+/, '').trim())  // Nettoyer le formatage
            .filter(line => line.length > 0);  // Éliminer les lignes vides
        }
        
        // Si l'extraction ne trouve pas d'étapes (cas où l'IA ne met pas de puces), prendre les phrases
        if (steps.length === 0 && stepsText && stepsText[1]) {
          steps = stepsText[1]
            .split(/(?:\.|;)/)  // Séparer par phrases
            .map(s => s.trim())
            .filter(s => s.length > 10 && s.length < 100);  // Filtrer pour n'avoir que des phrases significatives
        }
        
        // Valeurs par défaut si l'extraction échoue
        const responseObject = {
          hint: hintMatch && hintMatch[1] ? hintMatch[1].trim() : "Analysez le modèle et les fréquences des caractères dans le texte chiffré.",
          explanation: explanationMatch && explanationMatch[1] ? explanationMatch[1].trim() : "Ce type de chiffrement suit des règles spécifiques qui peuvent être découvertes par analyse.",
          nextSteps: steps.length > 0 ? steps : [
            "Identifiez les caractères les plus fréquents",
            "Cherchez des motifs répétitifs",
            "Essayez d'appliquer les techniques standard pour ce type de chiffrement"
          ],
          model: openAIService.getCurrentModelName()
        };
        ...
        // Fournir une réponse par défaut en cas d'erreur d'analyse
        res.json({
          hint: "Analysez le modèle et les fréquences des caractères dans le texte chiffré.",
          explanation: `Pour le chiffrement de type "${cipherType}", cherchez à identifier les modèles spécifiques.`,
          nextSteps: [
            "Identifiez les caractères les plus fréquents",
            "Cherchez des motifs répétitifs",
            "Essayez d'appliquer les techniques standard pour ce type de chiffrement"
      ...
      console.error('Erreur lors de la génération d\'indices pour CodeBreaker:', error);
