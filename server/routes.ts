...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/cyber/start-scenario', async (req, res) => {
    try {
      const { scenarioId, userName, config, userLevel } = req.body;
      
      if (!scenarioId || !userName) {
        return res.status(400).json({ message: 'Missing required parameters' });
      }
      
      // Utiliser le niveau d'expertise de l'utilisateur s'il est fourni, sinon utiliser "débutant" par défaut
      const expertiseLevel = userLevel || 'débutant';
      ...
      const messages: ChatCompletionRequestMessage[] = [
        ...
        },
        {
          ...
          - Le niveau d'expertise de l'utilisateur est: ${expertiseLevel} (adapte le contenu technique à ce niveau)
          ...
          - Le problème doit être concret, spécifique et adapté au secteur d'activité et au niveau d'expertise ${expertiseLevel}
          - Si niveau "débutant": utilise des termes simples et pose des questions basiques sur les fondamentaux
          - Si niveau "confirmé": présente des situations plus complexes nécessitant une analyse technique intermédiaire
          - Si niveau "senior": aborde des problématiques avancées nécessitant une expertise technique solide
          - Si niveau "expert": présente des scénarios sophistiqués impliquant des connaissances très pointues
...
Réponds directement à la première personne comme si tu étais ${supervisor.name} qui s'adresse au ${userRole}.`;
  ...
  // Routes pour l'évaluation du niveau en cybersécurité
  app.get('/api/cyber/assessment/questions', (req, res) => {
    const { domain } = req.query;
    
    // Questions adaptées par domaine
    const domainQuestions: Record<string, any> = {
      'Ingénierie sociale et phishing': [
        {
          id: 'phish-q1',
          text: 'Comment identifiez-vous un email de phishing sophistiqué ?',
          options: [
            { id: 'p1-a', text: 'Je vérifie simplement si l\'email contient des fautes d\'orthographe', level: 'débutant' },
            { id: 'p1-b', text: 'Je regarde l\'adresse d\'expéditeur et les liens au survol, mais je peux être trompé par des domaines similaires', level: 'confirmé' },
            { id: 'p1-c', text: 'J\'examine les en-têtes techniques, l\'adresse d\'expéditeur et j\'analyse les URLs complètes avant de cliquer', level: 'senior' },
            { id: 'p1-d', text: 'J\'utilise une approche multi-facteurs incluant l\'analyse des en-têtes DKIM/SPF, la vérification OSINT de l\'expéditeur et l\'inspection du code source', level: 'expert' }
          ]
        },
        {
          id: 'phish-q2',
          text: 'Face à une tentative d\'ingénierie sociale par téléphone, quelle est votre réaction ?',
          options: [
            { id: 'p2-a', text: 'Je raccroche dès que je suspecte une tentative d\'arnaque', level: 'débutant' },
            { id: 'p2-b', text: 'Je ne communique aucune information sensible et je contacte le service informatique', level: 'confirmé' },
            { id: 'p2-c', text: 'Je suis des procédures formelles de vérification d\'identité et je documente l\'incident', level: 'senior' },
            { id: 'p2-d', text: 'J\'engage l\'appelant tout en recueillant des informations pour l\'analyse forensique, suivant un protocole de gestion d\'incident établi', level: 'expert' }
          ]
        },
        {
          id: 'phish-q3',
          text: 'Quelle méthode utiliseriez-vous pour sensibiliser une entreprise au phishing ?',
          options: [
            { id: 'p3-a', text: 'Envoyer des emails explicatifs avec les bonnes pratiques à suivre', level: 'débutant' },
            { id: 'p3-b', text: 'Organiser des formations obligatoires avec exemples concrets', level: 'confirmé' },
            { id: 'p3-c', text: 'Mettre en place des campagnes de phishing simulées avec reporting et formations ciblées', level: 'senior' },
            { id: 'p3-d', text: 'Déployer un programme complet incluant simulations adaptatives, analyses comportementales et mesures correctives personnalisées', level: 'expert' }
          ]
        }
      ],
      'Stratégie et gouvernance cybersécurité': [
        {
          id: 'strat-q1',
          text: 'Comment établiriez-vous les priorités de sécurité dans une organisation ?',
          options: [
            { id: 's1-a', text: 'Je me concentre sur les problèmes les plus visibles et récents', level: 'débutant' },
            { id: 's1-b', text: 'J\'utilise une approche basée sur les risques identifiés dans l\'actualité cyber', level: 'confirmé' },
            { id: 's1-c', text: 'Je réalise une analyse de risques formelle et j\'aligne les contrôles sur les normes de l\'industrie', level: 'senior' },
            { id: 's1-d', text: 'Je combine analyse quantitative des risques, alignement stratégique avec le business, et anticipation des menaces émergentes', level: 'expert' }
          ]
        },
        {
          id: 'strat-q2',
          text: 'Comment mesurez-vous l\'efficacité d\'une stratégie de cybersécurité ?',
          options: [
            { id: 's2-a', text: 'Par l\'absence d\'incidents de sécurité signalés', level: 'débutant' },
            { id: 's2-b', text: 'Par des indicateurs simples comme le nombre de vulnérabilités corrigées', level: 'confirmé' },
            { id: 's2-c', text: 'Via un tableau de bord de KPIs techniques et business alignés avec les objectifs de sécurité', level: 'senior' },
            { id: 's2-d', text: 'Par un système complet de métriques prédictives et réactives, avec ROI de sécurité et impact sur la résilience business', level: 'expert' }
          ]
        },
        {
          id: 'strat-q3',
          text: 'Comment intégreriez-vous la sécurité dans la culture d\'entreprise ?',
          options: [
            { id: 's3-a', text: 'En distribuant des guides de bonnes pratiques aux employés', level: 'débutant' },
            { id: 's3-b', text: 'Par des formations régulières et des communications de sensibilisation', level: 'confirmé' },
            { id: 's3-c', text: 'En développant un programme complet avec champions de sécurité et système de reconnaissance', level: 'senior' },
            { id: 's3-d', text: 'Par une transformation organisationnelle intégrant la sécurité dans la gouvernance, les objectifs de performance et l\'innovation', level: 'expert' }
          ]
        }
      ],
      'RGPD et conformité': [
        {
          id: 'rgpd-q1',
          text: 'Comment géreriez-vous une violation de données personnelles ?',
          options: [
            { id: 'r1-a', text: 'Je corrige le problème technique dès que possible', level: 'débutant' },
            { id: 'r1-b', text: 'J\'informe la CNIL et les personnes concernées dans les délais légaux', level: 'confirmé' },
            { id: 'r1-c', text: 'J\'active un plan de réponse structuré avec analyse d\'impact, notification légale et mesures correctives', level: 'senior' },
            { id: 'r1-d', text: 'J\'orchestre une réponse complète incluant forensique, communication de crise, analyse juridique, et amélioration systémique', level: 'expert' }
          ]
        },
        {
          id: 'rgpd-q2',
          text: 'Comment implémentez-vous le principe de minimisation des données ?',
          options: [
            { id: 'r2-a', text: 'En supprimant les données qui ne sont plus utilisées', level: 'débutant' },
            { id: 'r2-b', text: 'En limitant la collecte aux données strictement nécessaires', level: 'confirmé' },
            { id: 'r2-c', text: 'Par une stratégie de cycle de vie des données avec anonymisation et purge automatisée', level: 'senior' },
            { id: 'r2-d', text: 'Via une architecture de privacy by design avec cartographie dynamique, anonymisation différentielle et contrôles d\'accès contextuels', level: 'expert' }
          ]
        },
        {
          id: 'rgpd-q3',
          text: 'Comment abordez-vous l\'analyse d\'impact relative à la protection des données (AIPD) ?',
          options: [
            { id: 'r3-a', text: 'Je remplis le formulaire standard de la CNIL', level: 'débutant' },
            { id: 'r3-b', text: 'Je documente les risques et les mesures de protection pour chaque traitement', level: 'confirmé' },
            { id: 'r3-c', text: 'J\'applique une méthodologie structurée d\'analyse avec participation des parties prenantes et révision juridique', level: 'senior' },
            { id: 'r3-d', text: 'J\'intègre l\'AIPD dans le cycle de développement avec modélisation des menaces, évaluation quantitative et validation multi-disciplinaire', level: 'expert' }
          ]
        }
      ],
      'Sécurité des systèmes d\'information': [
        {
          id: 'sec-q1',
          text: 'Face à une vulnérabilité critique dans un système de production, quelle est votre approche ?',
          options: [
            { id: 'i1-a', text: 'J\'applique immédiatement le correctif disponible', level: 'débutant' },
            { id: 'i1-b', text: 'J\'évalue l\'impact du correctif avant de l\'appliquer en suivant la procédure de change', level: 'confirmé' },
            { id: 'i1-c', text: 'J\'implémente des contrôles compensatoires temporaires pendant l\'évaluation complète du correctif', level: 'senior' },
            { id: 'i1-d', text: 'Je développe une stratégie de mitigation multiniveau basée sur le risque business, avec contrôles temporaires et validation d\'intégration', level: 'expert' }
          ]
        },
        {
          id: 'sec-q2',
          text: 'Comment sécuriseriez-vous une infrastructure cloud ?',
          options: [
            { id: 'i2-a', text: 'En utilisant les paramètres de sécurité par défaut du fournisseur cloud', level: 'débutant' },
            { id: 'i2-b', text: 'En appliquant les recommandations de sécurité du fournisseur et en surveillant les accès', level: 'confirmé' },
            { id: 'i2-c', text: 'En implémentant une architecture Zero Trust avec IAM avancé, microsegmentation et surveillance continue', level: 'senior' },
            { id: 'i2-d', text: 'Via une approche cloud-native security intégrant Infrastructure as Code sécurisée, CSPM, CWPP et SecDevOps automatisé', level: 'expert' }
          ]
        },
        {
          id: 'sec-q3',
          text: 'Quelle est votre approche pour la détection des menaces avancées (APT) ?',
          options: [
            { id: 'i3-a', text: 'Je me fie à mon antivirus et mon firewall', level: 'débutant' },
            { id: 'i3-b', text: 'J\'utilise un SIEM pour centraliser et corréler les logs de sécurité', level: 'confirmé' },
            { id: 'i3-c', text: 'Je déploie des solutions EDR/XDR avec chasse aux menaces proactive et analyse comportementale', level: 'senior' },
            { id: 'i3-d', text: 'J\'implémente une détection multicouche avec threat intelligence, détection d\'anomalies par IA, honeypots et équipe SOC dédiée', level: 'expert' }
          ]
        }
      ],
      'Gestion des incidents': [
        {
          id: 'inc-q1',
          text: 'Comment organisez-vous la réponse à un incident de sécurité majeur ?',
          options: [
            { id: 'g1-a', text: 'Je résous le problème technique au plus vite', level: 'débutant' },
            { id: 'g1-b', text: 'Je suis un plan de réponse prédéfini avec des rôles assignés', level: 'confirmé' },
            { id: 'g1-c', text: 'J\'active une cellule de crise structurée avec investigation forensique et communication coordonnée', level: 'senior' },
            { id: 'g1-d', text: 'J\'orchestre une réponse multiniveau avec cellules stratégique/tactique/opérationnelle, playbooks adaptés et gestion de la chaîne de preuves', level: 'expert' }
          ]
        },
        {
          id: 'inc-q2',
          text: 'Comment préparez-vous l\'organisation à gérer des incidents ?',
          options: [
            { id: 'g2-a', text: 'En documentant les procédures à suivre', level: 'débutant' },
            { id: 'g2-b', text: 'En formant les équipes techniques et en réalisant des exercices de table', level: 'confirmé' },
            { id: 'g2-c', text: 'Par des simulations réalistes, des analyses post-exercice et l\'amélioration continue des procédures', level: 'senior' },
            { id: 'g2-d', text: 'Via un programme complet incluant exercices avancés (purple team), simulations inattendues et intégration avec la continuité d\'activité', level: 'expert' }
          ]
        },
        {
          id: 'inc-q3',
          text: 'Comment analysez-vous un incident pour éviter qu\'il ne se reproduise ?',
          options: [
            { id: 'g3-a', text: 'J\'identifie et corrige la vulnérabilité exploitée', level: 'débutant' },
            { id: 'g3-b', text: 'Je documente l\'incident et mets en place des correctifs pour les problèmes identifiés', level: 'confirmé' },
            { id: 'g3-c', text: 'Je réalise une analyse des causes profondes avec méthodologie structurée et plan d\'action complet', level: 'senior' },
            { id: 'g3-d', text: 'J\'applique des techniques d\'analyse systémique avec identification des défaillances techniques et organisationnelles, mesures préventives et transformation des processus', level: 'expert' }
          ]
        }
      ]
    };
    
    // Questions par défaut si le domaine n'est pas spécifié
    const defaultQuestions = [
      {
        id: 'gen-q1',
        text: 'Quel est votre niveau d\'expérience en cybersécurité ?',
        options: [
          { id: 'g1-a', text: 'J\'ai des connaissances de base acquises en autodidacte ou lors de formations courtes', level: 'débutant' },
          { id: 'g1-b', text: 'J\'ai 1 à 3 ans d\'expérience professionnelle ou une formation spécialisée', level: 'confirmé' },
          { id: 'g1-c', text: 'J\'ai 3 à 7 ans d\'expérience et des certifications reconnues (CISSP, CEH, etc.)', level: 'senior' },
          { id: 'g1-d', text: 'J\'ai plus de 7 ans d\'expérience et une expertise pointue dans plusieurs domaines de la cybersécurité', level: 'expert' }
        ]
      },
      {
        id: 'gen-q2',
        text: 'Comment restez-vous informé des nouvelles menaces et vulnérabilités ?',
        options: [
          { id: 'g2-a', text: 'Je consulte occasionnellement l\'actualité cybersécurité', level: 'débutant' },
          { id: 'g2-b', text: 'Je suis régulièrement les flux d\'information spécialisés et les bulletins de sécurité', level: 'confirmé' },
          { id: 'g2-c', text: 'J\'utilise des sources multiples incluant threat intelligence, communautés et veille technologique structurée', level: 'senior' },
          { id: 'g2-d', text: 'Je combine veille proactive, participation à des communities privées, recherche personnelle et analyse prédictive', level: 'expert' }
        ]
      },
      {
        id: 'gen-q3',
        text: 'Face à un problème de sécurité complexe et inconnu, quelle est votre approche ?',
        options: [
          { id: 'g3-a', text: 'Je cherche des solutions sur internet ou demande de l\'aide', level: 'débutant' },
          { id: 'g3-b', text: 'J\'analyse méthodiquement le problème et consulte la documentation technique appropriée', level: 'confirmé' },
          { id: 'g3-c', text: 'J\'utilise des techniques structurées de résolution de problèmes avec isolement des variables et test d\'hypothèses', level: 'senior' },
          { id: 'g3-d', text: 'Je combine reverse engineering, analyse forensique et développement d\'outils personnalisés si nécessaire', level: 'expert' }
        ]
      }
    ];
    ...
    let questions;
    if (domain && domainQuestions[domain as string]) {
      questions = domainQuestions[domain as string];
    } else {
  ...
  app.post('/api/cyber/assessment/evaluate', (req, res) => {
    const { domain, answers, questions } = req.body;
    ...
    // Pour chaque réponse, trouver l'option correspondante et incrémenter le compteur de niveau
    Object.entries(answers).forEach(([questionId, optionId]) => {
      const question = questions.find((q: any) => q.id === questionId);
      if (question) {
        const option = question.options.find((opt: any) => opt.id === optionId);
        if (option && option.level) {
          levelPoints[option.level]++;
        }
      }
    });
    
    // Déterminer le niveau prédominant
    let maxLevel = 'débutant';
    let maxPoints = 0;
    
    Object.entries(levelPoints).forEach(([level, points]) => {
      if (points > maxPoints) {
        maxPoints = points;
        maxLevel = level;
      }
    ...
    // En cas d'égalité, privilégier le niveau supérieur pour l'AGENT CONVERSATIONNEL
    const levels = ['débutant', 'confirmé', 'senior', 'expert'];
    for (let i = levels.length - 1; i >= 0; i--) {
      if (levelPoints[levels[i]] === maxPoints) {
        maxLevel = levels[i];
        break;
      }
    }
