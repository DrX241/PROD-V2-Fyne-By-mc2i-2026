...
3. Ne pas inclure de messages privés ni de décisions, seulement la narration générale.`;
    ...
    // Utilisation correcte de la fonction avec tous les paramètres nécessaires
    const completion = await openAIService.getChatCompletion(
      game.aiContext,   // messages
...
// Fonction helper pour extraire un JSON d'une réponse en texte
const extractJSON = (text: string) => {
  try {
    // Recherche d'un objet JSON dans le texte
...
Assure-toi que les options soient pertinentes pour le rôle ${player.role} et pour ce tour ${turnNumber}.`;
      ...
      // Pour le JSON, nous utilisons une option spéciale responseFormat
      const completion = await openAIService.getChatCompletion(
        game.aiContext,
        true,
        0.7,
      ...
      if (completion) {
        ...
        const decisionData = extractJSON(completion);
        
        if (decisionData && decisionData.prompt && decisionData.options) {
          // Ajouter la réponse au contexte
          game.aiContext.push({
            role: 'assistant',
            content: completion
          ...
          const decisionMessage: GameMessage = {
            ...
            type: 'decision',
            targetPlayer: player.id,
            content: `**Décision à prendre**: ${decisionData.prompt}`,
            decisions: decisionData.options,
            turnNumber
          };
...
Format attendu: JSON avec cette structure:
{
  ...
  "metrics": {
    ...
    if (completion) {
      ...
      const consequenceData = extractJSON(completion);
      
      if (consequenceData && consequenceData.consequence && consequenceData.metrics) {
        // Ajouter la réponse au contexte
        game.aiContext.push({
          role: 'assistant',
          content: completion
        ...
        const consequenceMessage: GameMessage = {
          ...
          timestamp: Date.now(),
          type: 'system',
          content: `**Conséquence de la décision de ${player.name} (${player.role})** : ${consequenceData.consequence}`,
          turnNumber: game.gameState.currentTurn + 1
        };
        ...
        const metrics = consequenceData.metrics;
...
Format: Un texte structuré avec des titres clairs et des paragraphes.`;
    ...
    const completion = await openAIService.getChatCompletion(
      ...
      true,
      0.7,
      4000  // Plus long pour le résumé final
    );
