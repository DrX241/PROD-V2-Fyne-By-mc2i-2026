import { Request, Response } from 'express';
import { openAIService } from './services/openai';
// Définir nos propres types pour éviter les problèmes de compatibilité
type Role = "system" | "user" | "assistant";
interface MessageType {
  role: Role;
  content: string;
}

interface CyberPractice {
  text: string;
  isGood: boolean;
  explanation: string;
}

/**
 * Génère des pratiques de cybersécurité aléatoires (bonnes et mauvaises) via Azure OpenAI
 */
export async function generateCyberPractices(req: Request, res: Response) {
  try {
    const count = parseInt(req.query.count as string) || 10;
    
    // Configuration du prompt
    const systemPrompt = `Tu es un expert en cybersécurité. Génère un ensemble de pratiques de cybersécurité, 
    comprenant à la fois de bonnes et de mauvaises pratiques. 
    Pour chaque élément, fournis le texte (court, max 6 mots), indique s'il s'agit d'une bonne pratique (true) 
    ou d'une mauvaise pratique (false), et une brève explication (maximum 20 mots) sur pourquoi c'est une bonne ou 
    mauvaise pratique. Réponds UNIQUEMENT avec un tableau JSON sans aucun texte avant ou après.`;

    const userPrompt = `Génère exactement ${count} pratiques de cybersécurité aléatoires, 
    avec exactement 50% de bonnes pratiques (isGood: true) et 50% de mauvaises pratiques (isGood: false). 
    
    Format attendu pour chaque pratique:
    {
      "text": "Courte description (max 6 mots)",
      "isGood": true/false,
      "explanation": "Explication courte (max 20 mots)"
    }
    
    Réponds uniquement avec un tableau JSON sans commentaires ni délimiteurs markdown, par exemple:
    [{"text":"Texte 1","isGood":true,"explanation":"Explication 1"},{"text":"Texte 2","isGood":false,"explanation":"Explication 2"}]`;

    // Utiliser le service OpenAI avec la méthode correcte
    const messages: MessageType[] = [
      { role: "system", content: systemPrompt },
      { role: "user", content: userPrompt }
    ];
    
    // Obtenir la réponse formatée en JSON
    // Convertir nos messages au format attendu par le service
    const formattedMessages = messages.map(m => {
      return {
        role: m.role,
        content: m.content
      };
    });
    
    const responseContent = await openAIService.getChatCompletion(
      formattedMessages as any, // Convertir le type pour éviter les erreurs
      0.7,  // temperature
      1000  // maxTokens
    );

    // Extraction et nettoyage des données
    if (!responseContent) {
      throw new Error("Réponse d'Azure OpenAI vide");
    }

    // Nettoyer les délimiteurs markdown et extraire le JSON
    let cleanedResponse = responseContent.trim();
    
    // Supprimer les délimiteurs Markdown s'ils existent
    if (cleanedResponse.startsWith("```json")) {
      cleanedResponse = cleanedResponse.replace(/^```json\n/, "").replace(/\n```$/, "");
    } else if (cleanedResponse.startsWith("```")) {
      cleanedResponse = cleanedResponse.replace(/^```\n/, "").replace(/\n```$/, "");
    }
    
    // Parse le JSON 
    let practices;
    try {
      // Essayer d'abord de parser la réponse complète
      const jsonData = JSON.parse(cleanedResponse);
      
      // Vérifier si la structure est dans un sous-objet "practices"
      if (jsonData.practices && Array.isArray(jsonData.practices)) {
        practices = jsonData.practices;
      } else if (Array.isArray(jsonData)) {
        // Si la réponse est directement un tableau
        practices = jsonData;
      } else {
        throw new Error("Structure de données incorrecte");
      }
    } catch (parseError) {
      console.error("Erreur de parsing JSON:", parseError);
      
      // Utiliser des pratiques par défaut en cas d'erreur de parsing
      practices = [
        { text: "Mots de passe complexes", isGood: true, explanation: "Renforce la sécurité des comptes" },
        { text: "Même mot partout", isGood: false, explanation: "Risque de compromission multiple" },
        { text: "Mises à jour régulières", isGood: true, explanation: "Corrige les vulnérabilités connues" },
        { text: "Cliquer tous les liens", isGood: false, explanation: "Risque de phishing élevé" },
        { text: "Authentification à deux facteurs", isGood: true, explanation: "Ajoute une couche de protection" }
      ];
    }

    // Retour des pratiques
    res.json({ 
      success: true, 
      practices: practices 
    });
  } catch (error: any) {
    console.error("Erreur lors de la génération des pratiques de cybersécurité:", error);
    res.status(500).json({ 
      success: false, 
      error: error.message || "Erreur lors de la génération des pratiques" 
    });
  }
}